...
public class CharacterController2D : MonoBehaviour
{
    ...
    */
    ...
    [SerializeField] private Transform m_LadderCheck;                           // A position marking where to the starting point of ladder ray is.
    ...
    public void Move(float offsetX)
    {
        ...
        if (IsGrounded || m_AirControl)
        {
            ...
            if (offsetX > 0 && !m_FacingRight)
            {
                // ... flip the player.
                Flip();
            }
            // Otherwise if the input is moving the player left and the player is facing right...
            else if (offsetX < 0 && m_FacingRight)
            {
                // ... flip the player.
                Flip();
            }
